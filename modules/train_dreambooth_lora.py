...
paddlenlp.utils.log.logger.set_level("WARNING")
...
from helper.scheduler import clip_grad_norm_
import numpy as np
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.,
        help="validation_guidance_scale.",
    )
    parser.add_argument(
        "--instance_prompt",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Maybe we want to load pretrained lora wieght, we should give a lora_weight_or_path.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        "--use_lion", type=str2bool, nargs="?", const=False, help="use_lion."
    )
    parser.add_argument(
        "--lr_scheduler",
        type=str,
...
def main():
    ...
    if args.with_prior_preservation:
        ...
        if cur_class_images < args.num_class_images:
            ...
            for example in tqdm(
                sample_dataloader, desc="Generating class images", disable=not is_main_process, ncols=200
            ):
    ...
    # apply lora
    unet.apply_lora(
        multiplier=1.0, rank=args.lora_rank, alpha=None, lora_weight_or_path=args.lora_weight_or_path
    )  # 我们可以加载预训练的继续训练
    ...
    parmas = itertools.chain(unet.parameters(), text_encoder.parameters())

    if args.use_lion:
        from utils import Lion, get_scheduler
        optimizer = Lion(parmas, lr=args.learning_rate, weight_decay=args.adam_weight_decay,)
    ...
    progress_bar = tqdm(range(args.max_train_steps), disable=not is_main_process, ncols=200)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                if num_processes > 1 and args.gradient_checkpointing:
                    fused_allreduce_gradients(parmas, None)
                if args.use_lion and args.max_grad_norm>0:
                    clip_grad_norm_(parmas, args.max_grad_norm)
                optimizer.step()
                lr_scheduler.step()
                ...
                if is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        tmp_dict = {}
                        tmp_dict.update(lora_helper.extract_lora_weights(text_encoder))
                        tmp_dict.update(lora_helper.extract_lora_weights(unet))
                        paddle.save(tmp_dict, os.path.join(save_path, "text_encoder_unet_lora.pdparams"))
                        save_file(
                            lora_helper.convert_pd_to_pt(tmp_dict),
                            os.path.join(save_path, "text_encoder_unet_lora.safetensors"),
                        )
        ...
        if is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                images = [
                    pipeline(args.validation_prompt, num_inference_steps=25, generator=generator, guidance_scale=args.validation_guidance_scale).images[0]
    ...
    tmp_dict.update(lora_helper.extract_lora_weights(unet))
    paddle.save(tmp_dict, os.path.join(save_path, "text_encoder_unet_lora.pdparams"))
    save_file(lora_helper.convert_pd_to_pt(tmp_dict), os.path.join(save_path, "text_encoder_unet_lora.safetensors"))
    print(f"Saved final lora weights to {save_path}")
