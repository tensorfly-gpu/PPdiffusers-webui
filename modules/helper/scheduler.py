...
def clip_grad_norm_(
    ...
    # device = grads[0].place
    if norm_type == inf:
        norms = [g.abs().max() for g in grads]
        total_norm = norms[0] if len(norms) == 1 else paddle.max(paddle.stack(norms))
    else:
        total_norm = paddle.norm(paddle.stack([paddle.norm(g, norm_type) for g in grads]), norm_type)
...
def get_constant_schedule(optimizer: Optimizer, last_epoch: int = -1):
    ...
    Return:
        `lr_scheduler.LambdaLR` with the appropriate schedule.
    """
